...
Enemy.prototype.update  = function (dt) {
    ...
    if (this.x < player.x + minDistance && this.x + minDistance > player.x && this.y < player.y + minDistance && minDistance + this.y > player.y) {
...
Enemy.prototype.dieOut = function (dt) {
    ...
    ctx.drawImage(Resources.get(this.sprite), this.x, this.y);
    if (fadeOutAlpha <= 0) {
        // Add score according to how many bugs go off the screen if the player is
        //alive
        if (gameReset === 0) {
...
var insertEnemies = function (enemiesQtd, speed) {
    ...
    while (enemyCount < enemiesQtd) {
        allEnemies.push(new Enemy(-98, Math.floor(enemyPositionsY[Math.floor(Math.random() * 3)]), speed));
        enemyCount++;
